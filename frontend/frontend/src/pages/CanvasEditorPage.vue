...
const chatModelOptions = ['banana2', 'banana pro', 'GPT imag 2', 'agnes-image-2.1-flash'];
...
function buildElementLocationHint() {
  ...
  for (const el of selected) {
    ...
    const layerIdx = layers.value.findIndex((l) => l.id === el.layerId);
    const lName = `图层${layerIdx >= 0 ? ['一','二','三','四','五','六','七','八','九','十'][layerIdx] || layerIdx + 1 : '?'}`;
    lines.push(`  - ${el.name || '元素'}（在图层"${lName}"，检测框：[${pxLeft}, ${pxTop}, ${pxRight}, ${pxBottom}]）`);
  }
  lines.push('\n请重点针对以上选中的元素进行修改或替换，保持其他部分不变。');
  return lines.join('\n');
}
...
async function sendChat() {
  ...
  const locationHint = buildElementLocationHint();
  const fullPrompt = locationHint ? `${text}\n${locationHint}` : text;
  ...
  <main class="editor">
    ...
    <section class="editor-body">
      ...
      <aside v-if="activeTool === 'annotate'" class="annotate-banner">
        ...
        <button type="button" class="annotate-banner-close" @click="activeTool = 'select'">退出标记</button>
      </aside>
      ...
      >
        ...
        >
          ...
          <template v-if="layer.type === 'placeholder'">
            ...
            >
              ...
              <button class="uc-placeholder-close" type="button" title="移除这张执行卡（不影响后台任务）" @pointerdown.stop @click.stop.exact="removeLayer(layer.id)">×</button>
            </div>
          </template>
          <template v-else>
        ...
        <div v-if="manualNameInput.visible" class="manual-name-input" :style="{ left: `${manualNameInput.x}px`, top: `${manualNameInput.y}px` }" @pointerdown.stop>
          ...
          />
          <button @click.prevent="confirmManualElementName">✓</button>
          <button @click.prevent="cancelManualElementName">✕</button>
        </div>
