  ...
  return [
    {
      ...
      prompts: {
        modelInput:
          '根据产品信息生成电商详情页首屏，产品主体清晰，标题简洁，背景干净，高清商业摄影质感。负向提示词：不要乱码文字，不要产品变形，不要杂乱背景。',
      },
    },
  ]
})
...
const isDetailFlow = computed(() =>
  ['detail-page', 'detail-clone'].includes(generation.value?.flow),
)
const detailActionDisabled = computed(
  () => isDetailFlow.value && generation.value?.stage === 'generating',
)
const detailFlowTitle = computed(() =>
  generation.value?.flow === 'detail-clone' ? '竞品复刻' : '详情页生成',
)
const detailDesignTitle = computed(() =>
  generation.value?.flow === 'detail-clone' ? '竞品复刻设计' : '详情页设计',
)
...
const productReferenceImages = computed(() => {
  ...
  return normalizeImageUrls(generation.value?.images || []).filter(
    (url) => !competitorUrls.has(url),
  )
})
...
const activeTaskPlan = computed(() => {
  const index = Math.max(
    0,
    (generation.value?.detailResults || []).findIndex((url) => !url),
  )
  return activeDetailPlan.value[index >= 0 ? index : 0] || activeDetailPlan.value[0] || {}
})
const activeTaskPrompt = computed(() => {
  const promptText =
    activeTaskPlan.value?.prompts?.modelInput ||
    activeTaskPlan.value?.prompts?.positive ||
    generation.value?.prompt ||
    ''
  return String(promptText).replace(/\s+/g, ' ').trim()
})
const completedDetailCount = computed(
  () => (generation.value?.detailResults || []).filter(Boolean).length,
)
...
function handleDetailConfigAction() {
  ...
  if (
    generation.value.stage === 'planning' ||
...
function taskProgress(status, current, providerProgress) {
  ...
  if (Number.isFinite(providerProgress))
    return Math.max(current || 0, Math.min(99, providerProgress))
...
async function submitImageTask(
  ...
  const body = {
    ...
    image_urls: imageUrlsOverride
      ? normalizeImageUrls(imageUrlsOverride)
      : normalizeImageUrls(payload.images),
  }
  ...
  try {
    ...
    generationTimer = window.setInterval(async () => {
      ...
      try {
        ...
        if (!generation.value) return
        generation.value.progress = taskProgress(
          status.status,
...
async function startDetailImages() {
  ...
  generation.value.statusText = '灵感信号已捕获，创意引擎启动中...'
  ...
  try {
    ...
    const submissions = await Promise.all(
      plans.map((plan) =>
        submitImageTask(
          generation.value,
          plan.prompts?.modelInput ||
            plan.prompts?.positive ||
            `${generation.value.prompt}，生成第${plan.index}屏电商详情页`,
          1,
          plan.imageUrls,
        ),
      ),
    ...
    generationTimer = window.setInterval(async () => {
      ...
      try {
        const statuses = await Promise.all(
          generation.value.detailTaskIds.map((taskId) => fetchImageTask(taskId)),
        )
        if (!generation.value) return
        ...
        generation.value.progress = Math.max(
          ...
          Math.min(
            99,
            Math.round((completed / statuses.length) * 100) || generation.value.progress + 4,
          ),
        )
  ...
  <main :class="['youmi-home', { 'is-expanded': railExpanded, 'is-workspace': generation }]">
      ...
      <template v-if="loggedIn">
        <div class="yh-user-dropdown" :class="{ open: userMenuOpen }">
          ...
          <div v-show="userMenuOpen" class="yh-user-menu">
            ...
            <button
              type="button"
              class="yh-user-menu-item yh-user-menu-logout"
              @click="handleLogout"
            >
    ...
    <section v-else :class="['yh-generation-page', { 'has-detail-config': isDetailFlow }]">
      <aside v-if="isDetailFlow" class="yh-detail-config">
        ...
        <div class="yh-config-scroll">
          <section v-if="isCloneFlow" class="yh-config-block">
            ...
            <p class="yh-config-help">
              请上传其他产品/爆款参考/竞品的参考图，勿与「产品图片」重复上传同一件货。
            </p>
            <div class="yh-config-thumb-row">
              <figure
                v-for="(url, index) in competitorReferenceImages.slice(0, 2)"
                :key="`competitor-${url}`"
              >
                <img :src="url" :alt="`竞品参考图 ${index + 1}`" />
              </figure>
              <button type="button" class="yh-config-add-thumb">
                ☁
                <span>点击/拖拽上传图片</span>
              </button>
            </div>
          ...
          <section class="yh-config-block yh-config-resolution">
            ...
            <div v-if="isCloneFlow" class="yh-config-thumb-row">
              <figure
                v-for="(url, index) in productReferenceImages.slice(0, 2)"
                :key="`product-${url}`"
              >
          ...
          <section class="yh-config-block">
            ...
            <div class="yh-config-segment">
              ...
              <button type="button" :class="{ active: (generation.quality || '2K') === '2K' }">
                2K
              </button>
          ...
          <section v-if="isCloneFlow" class="yh-config-block">
            ...
            <p class="yh-config-help">
              仅当竞品图中含有模特时本设置才会生效；竞品图中无模特则忽略本设置。
            </p>
          </section>

          <button v-if="!isCloneFlow" class="yh-config-advanced" type="button">
            高级设定 已跳过策略 · 模特图 · 风格参考 · 分屏构思
            <span>点击展开</span>
          </button>
        </div>
      ...
      <header class="yh-workspace-top">
        <button class="yh-workspace-switch" type="button">
          首页生成
          <span>⌄</span>
        </button>
      ...
      <section class="yh-generation-thread">
        <div v-if="isDetailFlow && generation.stage === 'planning'" class="yh-detail-stepper">
          ...
          <span :class="{ active: generation.stage === 'generating', done: generation.done }">
            <i>{{ activeDetailPlan.length }}</i>
            生图
            <small>
              {{
                generation.done ? '已完成' : generation.stage === 'generating' ? '生成中' : '待生成'
              }}
            </small>
          </span>
        </div>
        ...
        <div class="yh-thread-head">
          ...
          <span
            v-if="!generation.done"
            :class="['yh-progress-pill', { failed: generation.stage === 'failed' }]"
          >
            {{ generation.stage === 'failed' ? '生成失败' : `生成中 ${generation.progress}%` }}
          </span>
          <span v-else class="yh-count-pill">
            {{ isDetailFlow ? activeDetailPlan.length : 1 }} 张
          </span>
        </div>

        <div class="yh-thread-meta">
          <img :src="referenceImageUrl" alt="参考图" />
          <span>
            {{ generation.ratio }} | img2 | {{ generation.quality }} | "{{ generation.prompt }}"
          </span>
        </div>
        ...
        <section v-if="isDetailFlow && generation.stage === 'planning'" class="yh-detail-plan">
          <header>
            <h2>
              {{ detailFlowTitle }}策划
              <small>共 {{ activeDetailPlan.length }} 屏</small>
            </h2>
            <div>
          ...
          <p class="yh-plan-tip">
            ⓘ
            点击卡片内容即可直接编辑。重新策划整套会保留历史版本，可用版本序号切换；也可多选若干屏仅重策该部分。
          </p>
          <div class="yh-plan-topic">
            <span>整套策略</span>
            <strong>
              {{ generation.style || '围绕产品信息、参考图和目标平台，保持统一视觉语言与转化节奏' }}
            </strong>
          </div>
          <article
            v-for="item in activeDetailPlan"
          ...
          >
            ...
            <div class="yh-plan-row">
              ...
              <p>
                {{
                  item.prompts?.modelInput ||
                  item.prompts?.positive ||
                  '根据本屏规划生成完整 9:16 电商详情页单屏。'
                }}
              </p>
            </div>
          </article>
        ...
        <section v-else-if="isDetailFlow" class="yh-detail-workbench">
          <div v-if="generatedPreviewUrls.length" class="yh-detail-preview-strip">
            <figure
              ...
              :key="`preview-${url}`"
              :class="{
                active: index === Math.min(completedDetailCount, generatedPreviewUrls.length - 1),
              }"
            >
              <img :src="url" :alt="`详情页预览 ${index + 1}`" />
              <span v-if="index === 0">详情页设计{{ activeTaskPlan.index || 1 }}⌄</span>
              <i v-if="index === Math.min(completedDetailCount, generatedPreviewUrls.length - 1)">
                ⌃
              </i>
            </figure>
          </div>
          ...
          <div class="yh-detail-stage">
              ...
              :class="{
                hidden:
                  index !==
                    Math.max(
                      0,
                      (generation.detailResults || []).findIndex((url) => !url),
                    ) && !generation.done,
              }"
            >
              <img
              ...
              <div
                v-else
                class="yh-detail-rendering-card"
                :style="{ '--progress': `${generation.progress}%` }"
              >
                <svg
                ...
                <span>
                  {{
                    generation.stage === 'failed'
                      ? '生成失败'
                      : `${Math.min(99, generation.progress)}%`
                  }}
                </span>
                <p>
                  {{
            ...
            <div v-if="!generation.done" class="yh-detail-screen-count">
              ⌄
              <span>{{ completedDetailCount }}/{{ activeDetailPlan.length }}</span>
            </div>
          </div>
        </section>
        ...
        <article v-else-if="!generation.done" class="yh-generation-card">
          ...
          <div class="yh-generating-center">
            ...
            <strong>{{ Math.min(99, generation.progress) }}%</strong>
