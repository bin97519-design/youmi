...
const shortcuts = [
  ...
  { title: '电商套图', desc: '一站式电商套图生成', icon: 'ecommerce-set' },
...
function handleShortcut(item) {
  ...
  if (item.icon === 'ecommerce-set') {
    router.push('/ecommerce-set')
    return
  }
      ...
      <span class="yh-shortcut-default">
        <span class="yh-shortcut-icon" aria-hidden="true">
          ...
          <svg v-else-if="item.icon === 'camera'" viewBox="0 0 24 24">
            <path
              d="M6 9.2h2.5l1.2-2h4.6l1.2 2H18c.7 0 1.2.5 1.2 1.2v6.4c0 .7-.5 1.2-1.2 1.2H6c-.7 0-1.2-.5-1.2-1.2v-6.4c0-.7.5-1.2 1.2-1.2Z"
            />
            <circle cx="12" cy="13.7" r="2.7" />
          </svg>
          <svg v-else-if="item.icon === 'ecommerce-set'" viewBox="0 0 24 24">
          ...
          <svg v-else viewBox="0 0 24 24">
            <path
              d="M8.2 5.6 5.1 8.2l2 3.1 1.7-1.1v8.2h6.4v-8.2l1.7 1.1 2-3.1-3.1-2.6-2.3 1.2h-3l-2.3-1.2Z"
            />
          </svg>
        </span>
        <span>
