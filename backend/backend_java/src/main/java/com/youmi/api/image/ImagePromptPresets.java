...
final class ImagePromptPresets {
  private static final String STYLE_TRANSFER = String.join("\n",
      ...
      "必须先识别并提取图1中的完整产品主体或产品组合，再把它完整、清晰地放入画面核心区域，占画面约45%至70%；不得遗漏产品，不得只生成图2场景。",
      "图1中的营销文案、价格条、边框和背景都不是产品的一部分，除非补充要求明确指定，否则不要复制。",
  ...
  private static final String LAYOUT_CLONE = String.join("\n",
      ...
      "必须用图1产品替换图2中的原商品或视觉主体。图1产品应占据画面核心区域约45%至70%，完整、清晰、无遮挡，不得遗漏产品或只生成图2场景。",
      "图1中的营销文案、价格条、边框和背景都不是产品的一部分，除非补充要求明确指定，否则不要复制。",
  ...
  private static boolean requestsNoText(String extra) {
    if (extra == null || extra.isBlank()) return false;
    String normalized = extra.toLowerCase();
    return normalized.contains("不要文字")
        || normalized.contains("不要任何文字")
        || normalized.contains("不需要文案")
        || normalized.contains("无文字")
        || normalized.contains("纯图")
