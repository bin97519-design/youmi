...
public class DetailCloneService {
  ...
  private static final int MAX_VISION_COMPETITOR_IMAGES = 40;
  private static final int MAX_VISION_PRODUCT_IMAGES = 8;
  ...
  private String uploadBufferedImage(BufferedImage image, String batchId, int index) throws Exception {
    ...
    HttpRequest request = HttpRequest.newBuilder(URI.create(imageProperties.getUploadEndpoint().trim()))
        .timeout(Duration.ofSeconds(Math.max(10, imageProperties.getUploadTimeoutSeconds())))
  ...
  private String deconstructPrompt(DetailCloneDtos.DeconstructRequest request) {
    return """
        请按 style-detail-clone 工作流分析竞品 A 的详情页图片，给每一张输入图片写 A Slice Contract。
        核心原则：复刻 A 的信息结构和视觉语法，不复刻 A 的事实声明。
        ...
        规则：
        1. index 必须对应输入图片顺序，从 1 开始，尽量一图一条契约。
        2. role 写这一屏的独立职责，例如首屏引导、材质证明、结构拆解、场景使用、对比升级、规格参数、信任转化。
        3. layoutSkeleton 写版式骨架：上下/左右结构、卡片数量、主体位置、标题位置、对比关系、留白节奏。
        4. visualLanguage 写视觉语法：背景、色彩、光影、卡片、标注线、图标、材质特写、场景风格。
        5. textHierarchy 写文字层级，不要照抄竞品文案，可描述为主标题/副标题/短卖点/参数表/角标。
        6. subjectRole 写产品或人物在画面里的角色。
        7. portableElements 只写可迁移元素：布局、节奏、构图、对比方式、信息顺序。
        8. claimsToDrop 必须列出要丢弃的竞品专属内容：品牌、证书、排名、价格、功效承诺、竞品商品事实。
        9. generationScope 只能是 scene-regeneration、style-transfer、text-only、product-replacement、full-regeneration 之一。
        10. 每个切片必须职责独立，不要把一个漂亮版式重复套用到全部切片。
        11. 复刻强度：%s。

        产品 C 信息仅用于判断迁移边界，不要写入 A 契约：
        %s
  ...
  private String mappingPrompt(
    ...
    return """
        请按 style-detail-clone 工作流，把产品 C 的真实信息映射到竞品 A 的切片契约中。
        核心原则：保留 A 的结构和视觉语法；替换成 C 的真实产品、真实卖点、真实场景；删除 A 的品牌和事实声明。
        ...
        2. replaceWithProduct 必须来自产品 C Markdown 信息或产品图片；不可编造证书、销量、排名、价格或医疗功效。
        3. variableSlots 要填可直接用于详情页的中文标题、卖点、参数、场景文案；如果某字段未知，用柔性表达，不能写假参数。
        4. forbidden 必须包含竞品品牌、竞品事实、未验证奖项/证书/排名/价格、医疗治疗或夸大功效。
        5. generationHint 必须是给图生图模型的完整中文提示词，明确：参考竞品图只借鉴布局和视觉语法，产品外观以产品 C 图片为准。
        6. 每屏提示词要独立，和该屏职责匹配；每屏必须有不同的信息角色。
        7. 如果 A 某屏是证书、排名、价格、竞品专属证明，而 C 没有真实依据，应改成软性信任/材质/工艺/参数/服务表达，或在 forbidden 里删除该声明。
        8. 复刻强度：%s。
        """.formatted(
        productInfo,
  ...
  private List<String> limitedList(List<String> values, int max) {
    if (max <= 0) return List.of();
    return safeList(values).stream().limit(max).toList();
  }
}
