...
function isActiveItem(item) {
  ...
  return openGroup.value === item.key;
}
  ...
  <aside :class="['yh-rail', { 'is-expanded': expanded }]">
    <div class="yh-rail-brand">
      <svg class="yh-logo-mark" viewBox="0 0 122 34" aria-label="YOUMI">
        ...
        <path class="logo-fill" d="M36.2 8.1c8.4 0 14.1 4.9 14.1 11.3 0 5.8-4.9 10.1-11.4 10.1-7.6 0-13.1-4.9-13.1-11.2 0-5.9 4.5-10.2 10.4-10.2Zm.8 6.3c-2.5 0-4.3 1.8-4.3 4.2 0 2.8 2.6 4.9 5.8 4.9 2.9 0 5-1.8 5-4.1 0-2.8-2.8-5-6.5-5Z" />
        <path class="logo-stroke" d="M51.7 10.2v9.4c0 6 3.3 9.6 9.1 9.6 5.8 0 9.5-3.8 9.5-9.8V10.2" />
      ...
      <div v-for="item in menuItems" :key="item.key" class="yh-nav-group">
        ...
        >
          <span class="yh-nav-icon" aria-hidden="true">
            ...
            <svg v-else-if="item.icon === 'canvas'" class="yh-icon yh-icon-canvas" viewBox="0 0 24 24">
            ...
            <svg v-else-if="item.icon === 'designer'" class="yh-icon" viewBox="0 0 24 24">
              ...
              <path class="white" d="M5.6 6.2 7 3.8l1.4 2.4 2.5 1.4L8.4 9 7 11.5 5.6 9 3.1 7.6l2.5-1.4Z" />
            </svg>

            <svg v-else-if="item.icon === 'product'" class="yh-icon" viewBox="0 0 24 24">
              <path class="white" d="M5.5 6.5h13v12h-13z" />
              <path class="cyan" d="M8 4.1v4.8M16 4.1v4.8M8.2 13.8l2.4-2.5 2.4 2.4 1.6-1.5 2.7 3.1" />
          ...
          <span v-if="expanded && item.children" :class="['yh-nav-arrow', { open: openGroup === item.key }]">
        ...
        <div v-if="expanded && item.children && openGroup === item.key" class="yh-submenu">
          <button
            ...
            :key="child.label"
            :class="['yh-sub-item', { active: child.active, locked: child.locked }]"
            type="button"
    ...
    <div class="yh-rail-history">
      <button :class="['yh-nav-item', { active: route.path === '/history' }]" type="button" @click="openHistory">
        <span class="yh-nav-icon" aria-hidden="true">
      ...
      <button :class="['yh-nav-item', { active: route.path === '/console' }]" type="button" @click="openConsole">
        <span class="yh-nav-icon" aria-hidden="true">
          <svg class="yh-icon" viewBox="0 0 24 24">
    ...
    <div class="yh-rail-bottom">
      <template v-if="!loggedIn">
        <button v-if="expanded" class="yh-login-wide" type="button" @click="emit('login')">登录</button>
        <button class="yh-bottom-icon" type="button">♧</button>
        <button v-if="!expanded" class="yh-login-mini" type="button" @click="emit('login')">登录</button>
      </template>
