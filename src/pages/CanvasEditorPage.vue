...
function getPortPosition(layerId, port) {
  ...
  // 查找该节点内对应方向的端口 DOM
  const layerEl = stageEl.querySelector(`[data-layer-id="${layerId}"]`)
  const portEl = layerEl?.querySelector(`.uc-port-${port}`)
  if (portEl) {
  ...
  const nodeHeight =
    ...
    (layer.naturalWidth && layer.naturalHeight
      ...
      : 150)
  const portCenterVisualOffset = 16 / vs
...
function addGeneratingPlaceholderLayer(prompt) {
  ...
  canvas.updateDocument(props.id, (draft) => {
    ...
    const fallbackY = cy - placeholderHeight / 2
    const layer = {
...
async function replaceGeneratingPlaceholder(layerId, url) {
  ...
  try {
    ...
    canvas.updateDocument(props.id, (draft) => {
      ...
      // 跟在选中图层后面（右下偏移）
      const base = [...layers.value]
        .reverse()
        .find((l) => l.id !== layerId && l.type !== 'placeholder')
      const cx = (viewportSize.width / 2 - viewOffset.value.x) / viewScale.value
      const cy = (viewportSize.height / 2 - viewOffset.value.y) / viewScale.value
...
function resizeLayer(event) {
  ...
  const width = Math.max(
    60,
    resizeState.value.width + (resizeState.value.point.includes('left') ? -dx : dx),
  )
  const height = Math.round(width * ratio)
  const x = resizeState.value.point.includes('left')
    ? resizeState.value.x + resizeState.value.width - width
    : resizeState.value.x
  const y = resizeState.value.point.includes('top')
    ? resizeState.value.y + resizeState.value.height - height
    : resizeState.value.y
  updateLayer(resizeState.value.id, {
    x: Math.round(x),
    y: Math.round(y),
...
function stopResize(event) {
  if (!resizeState.value) return
  if (event.currentTarget.hasPointerCapture(resizeState.value.pointerId))
    event.currentTarget.releasePointerCapture(resizeState.value.pointerId)
        ...
        >
          ...
          >
            <div class="layer-toolbar">
              <button>✂ 智能抠图</button>
              <button @click.stop="maybeAutoDetect(selectedLayer)">
          ...
          >
            ...
            >
              ...
              <footer class="uc-bottom-toolbar">
                ...
                <button
                  :disabled="
                    (!chatText.trim() && !getSelectedDetectedElements().length) || chatGenerating
                  "
                  @click="sendChat"
                >
