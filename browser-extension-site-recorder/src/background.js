...
const MAX_TEXT = 6000;
const SNAPSHOT_MENU_ID = 'youmi-save-complete-snapshot';
...
async function sendTabMessage(tab, message) {
  ...
  const isSnapshotMessage = String(message?.type || '').startsWith('youmi:snapshot:');
  try {
    const response = await chrome.tabs.sendMessage(tab.id, message);
    if (response !== undefined || !isSnapshotMessage) return response;
    await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ['src/snapshot.js'] });
    return await chrome.tabs.sendMessage(tab.id, message);
  } catch {
    try {
      const files = isSnapshotMessage ? ['src/snapshot.js'] : ['src/snapshot.js', 'src/content.js'];
...
async function sendMessageWithContentScript(tab, message) {
  return Boolean(await sendTabMessage(tab, message));
}
...
async function captureViewportScreenshot(tab) {
  ...
  return {
    ...
    height: null,
    scale: 1,
    warning: '未获得 Chrome 调试权限，仅保存了当前视口截图。',
  };
}
...
async function captureFullPageScreenshot(tab) {
  ...
  try {
    ...
    if (!hasDebuggerPermission) return await captureViewportScreenshot(tab);
    ...
    return {
      ...
      sourceHeight: height,
      scale,
      warning: scale < 1 ? `页面过大，完整截图按 ${Math.round(scale * 100)}% 缩放保存。` : '',
    };
  } catch (error) {
    const fallback = await captureViewportScreenshot(tab);
    fallback.warning = `完整页面截图失败，已退回当前视口：${String(error?.message || error)}`;
    return fallback;
  } finally {
...
async function runSnapshotCapture(tab, options = {}) {
  if (!tab?.id) throw new Error('没有找到当前网页标签页');
  if (/^(chrome|edge|about|devtools|chrome-extension):/i.test(tab.url || '')) {
    throw new Error('Chrome 内部页面不允许扩展采集');
  }
  if (snapshotCapture.running) throw new Error('正在生成网页快照，请稍后再试');

  snapshotCapture.running = true;
  try {
    await notifyReversePromptTab(tab, '正在采集 DOM、样式和页面状态...', 'running');
    const collected = await sendTabMessage(tab, {
      type: 'youmi:snapshot:collect',
      options: {
        includeScreenshot: options.includeScreenshot !== false,
        includeRecentEvents: options.includeRecentEvents !== false,
      },
    });
    if (!collected?.ok || !collected.snapshot) {
      throw new Error(collected?.error || '页面快照采集失败');
    }

    const pageEvents = options.includeRecentEvents === false
      ? []
      : memory.events.filter((event) => event.tabId === tab.id || event.pageUrl === collected.snapshot.page.url).slice(-500);
    collected.snapshot.interactions = {
    ...
    if (options.includeScreenshot !== false) {
      await notifyReversePromptTab(tab, '正在捕获页面截图...', 'running');
      screenshot = await captureFullPageScreenshot(tab);
      collected.snapshot.screenshot = {
        file: 'screenshot.png',
      ...
      if (screenshot.warning) collected.snapshot.warnings.push(screenshot.warning);
    }

    await notifyReversePromptTab(tab, '正在打包网页快照...', 'running');
    const downloaded = await sendTabMessage(tab, {
      type: 'youmi:snapshot:download',
      payload: {
        snapshot: collected.snapshot,
        screenshotBase64: screenshot?.data || '',
      },
    });
    if (!downloaded?.ok) throw new Error(downloaded?.error || '快照文件下载失败');

    const metadata = {
      capturedAt: collected.snapshot.capturedAt,
      pageUrl: collected.snapshot.page.url,
      title: collected.snapshot.page.title,
      filename: downloaded.filename,
      size: downloaded.size,
      summary: collected.snapshot.summary,
      warnings: collected.snapshot.warnings,
    };
    await chrome.storage.local.set({ youmiSnapshotLast: metadata });
    await notifyReversePromptTab(tab, `网页快照已保存：${downloaded.filename}`, 'done');
    return metadata;
  } finally {
    snapshotCapture.running = false;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message?.type === 'recorder:captureSnapshot') {
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      try {
        const metadata = await runSnapshotCapture(tab, message.options || {});
    ...
    if (message?.type === 'recorder:getLastSnapshot') {
      const data = await chrome.storage.local.get(['youmiSnapshotLast']);
      sendResponse({ ok: true, snapshot: data.youmiSnapshotLast || null });
      return;
    }
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (!tab?.id) return;
  if (info.menuItemId === SNAPSHOT_MENU_ID) {
    try {
      await runSnapshotCapture(tab, { includeScreenshot: true, includeRecentEvents: true });
